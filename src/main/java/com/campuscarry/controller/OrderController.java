...
import com.campuscarry.response.ApiResponse;
...
public class OrderController {
    ...
    @GetMapping("/feed")
    @PreAuthorize("hasRole('STUDENT')")
    public ResponseEntity<ApiResponse<Page<OrderFeedItemDto>>> getOrderFeed(
    ...
    public ResponseEntity<ApiResponse<OrderResponseDto>> acceptOrder(
        ...
        return ResponseEntity.ok(ApiResponse.success("Order accepted. OTP sent to requester.", response));
    }
    ...
    @PreAuthorize("hasRole('STUDENT')")
    public ResponseEntity<ApiResponse<OrderResponseDto>> notifyArrival(
            @PathVariable UUID id,
            @AuthenticationPrincipal User currentUser) {

        OrderResponseDto response = orderService.notifyArrival(id, currentUser.getId());
        return ResponseEntity.ok(ApiResponse.success("Requester notified of your arrival.", response));
    }
    ...
    @PreAuthorize("hasRole('STUDENT')")
    public ResponseEntity<ApiResponse<OrderResponseDto>> confirmDelivery(
            @PathVariable UUID id,
        ...
        return ResponseEntity.ok(ApiResponse.success("Delivery confirmed successfully.", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<OrderResponseDto>>> getMyOrders(
        ...
        return ResponseEntity.ok(ApiResponse.success("Orders loaded", orders));
    }
}